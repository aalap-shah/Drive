...
import com.agreeya.memoir.services.PhotoShotService.ImageBytes;
...
public class ImageLoader{
	...
	//for making bitmap from byte array
	public void loadImage(ImageBytes imgByts,int loadPriority, int expireTime){
		
		LoadImage(null,null,null,loadPriority,expireTime,"default",1);
	}
	/**
...
//		Log.d(TAG, "imageUrl > " + imageUrl + "   imageView > " + imageView);
		...
		File f = new File(fileDir,imageUrl);
		if (f.exists()) {
...
//				bitmap = BitmapFactory.decodeStream(new URL(url).openConnection().getInputStream(), null, options);
				BitmapFactory.Options optn = new Options();
				optn.inPurgeable=true;
				bitmap = BitmapFactory.decodeFile(url,optn);
			} catch (Exception e) {
				e.printStackTrace();
			}
			return bitmap;
	 ...
	 */
	private class SaveImageTask extends AsyncTask<ImageBytes, imageObject, Void> {

		@Override 
		protected Void doInBackground(ImageBytes... bitmapData) {
				...
				if((iO = (imageObject) saveQueue.poll()) != null) {
					try {
						...
						if(iO.imageBitmap != null) {
							...
							iO.imageBitmap.compress(Bitmap.CompressFormat.JPEG, 100, fileOS);
						} else {
